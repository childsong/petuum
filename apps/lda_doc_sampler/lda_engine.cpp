...
/////
#include <map>
std::map<int, std::vector<int> > word_topic_tbl;
////
...
void LDAEngine::Start() {
  ...
  while (true) {
    ...
    for (LDADocument::WordOccurrenceIterator it(&(*doc_iter));
      ...
      /////
      word_topic_tbl[it.Word()].resize(K_);
      CHECK_GT(K_, it.Topic());
      ++(word_topic_tbl[it.Word()][it.Topic()]);
      /////
    }
  }
  ...
  {
    //// verify word-topic table.
    int V = 53485;
    int num_words_seen = 0;
    for (int w = 0; num_words_seen < V && w < 2 * V; ++w) {
      petuum::RowAccessor word_topic_row_acc;
      word_topic_table.Get(w, &word_topic_row_acc);
      const auto& word_topic_row =
        word_topic_row_acc.Get<petuum::SortedVectorMapRow<int32_t> >();
      if (word_topic_row.num_entries() > 0) {
        CHECK(word_topic_tbl.find(w) != word_topic_tbl.end())
          << "PS word not found in word_topic_tbl";
        for (petuum::SortedVectorMapRow<int>::const_iterator wt_it =
            word_topic_row.cbegin(); !wt_it.is_end(); ++wt_it) {
          int count = wt_it->second;
          CHECK_EQ(word_topic_tbl[w][wt_it->first], count) << "word-topic mismatch: "
            << " for word " << w << " topic " << wt_it->first << " num_words_seen = "
            << num_words_seen;
        }
        ++num_words_seen;
      }
    }
    LOG(INFO) << "all word_topic_table items are represented in local word_topic_tbl";

    int num_word_pass = 0;
    for (auto it = word_topic_tbl.begin(); it != word_topic_tbl.end(); ++it) {
      int word_id = it->first;
      petuum::RowAccessor word_topic_row_acc;
      word_topic_table.Get(word_id, &word_topic_row_acc);
      const auto& word_topic_row =
        word_topic_row_acc.Get<petuum::SortedVectorMapRow<int32_t> >();
      CHECK(word_topic_row.num_entries() > 0);
      std::vector<int>& word_topic_vec = it->second;
      for (int i = 0; i < K_; ++i) {
        int count = 0;
        for (petuum::SortedVectorMapRow<int>::const_iterator wt_it =
            word_topic_row.cbegin(); !wt_it.is_end(); ++wt_it) {
        ...
        CHECK_EQ(count, word_topic_vec[i]) << "num_word_pass = " << num_word_pass;
      }
      ++num_word_pass;
    }
    LOG(INFO) << "Pass first word-topic table verification. num_word_pass = "
      << num_word_pass;
    ////
  }
  ...
  for (int iter = 0; iter < num_iterations; ++iter) {
...

    {
      ...
      LOG(INFO) << "Pass second word-topic table verification. num_word_pass = "
        << num_word_pass;
      ////
    }
