...
void StalenessTest(const ThreadContext &thread_context, int32_t thread_id) {
  ...
  for (iteration_num = 0; iteration_num < thread_context.num_iterations;
    ...
    // Test 2: read and verify other threads' cells
    for (table_id = 0; table_id < thread_context.num_tables; ++table_id) {

      petuum::RowAccessor row_acc_0;
      tables[table_id].Get(0, &row_acc_0);

      int32_t client_id = 0;
      int32_t client_app_thread_id_start = 0;
      for (client_id = 0; client_id < thread_context.num_clients;
	   ++client_id) {
        client_app_thread_id_start += thread_context.app_thread_id_offset;
	for (int i = 0; i < thread_context.num_app_threads_per_client;
	     ++i) {
          int32_t app_thread_id = client_app_thread_id_start + i;
	  int val = (row_acc_0.Get<petuum::DenseRow<int> >())[app_thread_id];
	  CHECK_GE(val, iteration_num - thread_context.staleness
                   + thread_context.resume_clock)
	    << "thread id = " << thread_id
	    << " row_id = " << 0
	    << " column id = " << app_thread_id;

	  petuum::RowAccessor row_acc_iter;
	  tables[table_id].Get(app_thread_id, &row_acc_iter);
	  val = (row_acc_iter.Get<petuum::DenseRow<int> >())[app_thread_id];
	  CHECK_GE(val, iteration_num - thread_context.staleness
                   + thread_context.resume_clock)
	    << "thread id = " << thread_id
	    << " row_id = " << app_thread_id
	    << " column id = " << app_thread_id;
	}
        client_app_thread_id_start +=
            thread_context.num_app_threads_per_client - 1;
      }
    }
